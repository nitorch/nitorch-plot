...
from nitorch_interpol import pull, affine_grid
from nitorch_io.utils.affine import (
    affine_default, compute_fov, affine_matvec,
    affine_inv, affine_rmdiv, affine_lmdiv,
...
def get_slice(image, dim=-1, index=None):
    ...
    return extra.slice_tensor(image, index, dim=dim)
...
def _get_default_native(affines, shapes):
    ...
    # default affines
    affines = [affine_default(shape) if affine is None else affine
               for shape, affine in zip(shapes, affines)]

    affines = extra.as_tensor(affines)
    shapes = extra.as_tensor(shapes)
    affines, shapes = extra.to_max_device(affines, shapes)
...
def _get_default_space(affines, shapes, space=None, bbox=None):
    ...
    if space is None:
        voxel_size = get_voxel_size(affines)
    ...
    if bbox is None:
        ...
        mn, mx = compute_fov(space, affines, shapes)
    else:
        voxel_size = get_voxel_size(space)
        mn, mx = extra.as_tensor(bbox)
        mn /= voxel_size
        mx /= voxel_size
...
def get_oriented_slice(image, dim=-1, index=None, affine=None,
    ...
    if index is None:
        ...
        index = affine_matvec(affine_inv(space), index)
    ...
    if ndim == 2:
        shift = [[1, 0, - mn[0] + 1],
                 [0, 1, - mn[1] + 1],
                 [0, 0, 1]]
        shift = extra.as_tensor(shift, **backend)
    ...
    # sample
    space = affine_rmdiv(space, shift)
    affine = affine_lmdiv(affine, space)
    affine = affine.to(**backend)
    grid = affine_grid(affine, [*shape] + ([] if ndim == 2 else [1]))
    imshape = image.shape[-ndim:]
    channel = image.shape[:-ndim]
    image = image.reshape([-1, *imshape])
    image = pull(image.movedim(0, -1), grid, order=interpolation,
                 bound='dct2', extrapolate=False).movedim(-1, 0)
...
def show_orthogonal_slices(image, index=None, affine=None, fig=None,
    ...
    def transpose(x):
        return extra.movedim(x, [-2, -3], [-3, -2])
