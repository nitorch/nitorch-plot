...
from nitorch_core import constants, py, extra
from nitorch_core.hist import histc, _hist_to_quantile
from nitorch_fastmath import reduce, dot
...
def disp_to_rgb(image, vmax=None, scale=1, amplitude='value'):
    ...
    *batch, dim, height, width = image.shape
    ...
    # rescale intensities
    scale = extra.make_vector(scale, dim, **backend)[..., None, None]
    image = image * scale
    vmax = vmax or reduce.max(image.square().sum(-3).sqrt(), dim=[-1, -2])
...
def depth_to_rgb(image, colormap=None):
    ...
    *batch, depth, height, width = image.shape
    ...
    image = extra.movedim(image, -3, -1)
    cimage = dot(image.unsqueeze(-2), colormap.T)
...
def intensity_to_rgb(image, min=None, max=None, colormap='gray', n=256, eq=False):
    ...
    colormap = colormap.T.reshape([-1, 3])
    image = pull(colormap, image)
...
def intensity_preproc(*images, min=None, max=None, eq=None):
    ...
    *images : (*batch, H, W) tensor
    ...
    if len(images) == 1:
        images = [extra.to_max_backend(*images)]
    else:
        images = extra.to_max_backend(*images)
    backend = extra.backend(images[0])
    eps = constants.eps(images[0].dtype)

    # rescale min/max
    min = py.make_list(min, len(images))
    max = py.make_list(max, len(images))
    min = [extra.quantile(image, 0.05, bins=2048, dim=[-1, -2], keepdim=True)
    ...
    del othermin
    max = [extra.quantile(image, 0.95, bins=2048, dim=[-1, -2], keepdim=True)
    ...
    batch = extra.expanded_shape(*[image.shape[:-2] for image in images])
    ...
    if not (isinstance(eq, str) and eq.startswith('lin')):
        # rescale min/max
        images -= reduce.min(images, dim=-1, keepdim=True)
        images /= reduce.max(images, dim=-1, keepdim=True)
...
def histeq(x, n=1024, dim=None):
    ...
    min = reduce.min(x, dim=dim)
    max = reduce.max(x, dim=dim)
    batch_shape = min.shape
    hist = histc(x, n, dim=dim, min=min, max=max)
    ...
    shift = pull(shift.reshape([-1, n, 1]), grid,
                 bound='zero', extrapolate=True).squeeze(-1)
