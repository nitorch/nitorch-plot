...
from nitorch_io.utils.affine import (
    affine_inv as aff_inv, affine_matvec as aff_mv, affine_default,
    voxel_size as get_voxel_size, volume_layout, affine_reorient, compute_fov
)
...
class ImageArtist:
    ...
    def affine(self, value):
        if value is None:
            self._affine = affine_default(self.shape)
        self._affine = value
    ...
    def _map_image(self):
        ...
        if self._map is None:
            ...
            if aff is None:
                aff = affine_default(dat.shape[-3:])
            self._fdata = dat
...
class ImageViewer:
    ...
    def _index_from_cursor(self, x, y, image, n_ax):
        p = extra.as_tensor([x, y, 0])[:self.ndim]
    ...
    def on_move(self, event):
        ...
        if is_pressed_left and not is_pressed_shift:
            ...
            if last_position is None:
                return
            vx = get_voxel_size(self._space_matrix)
    ...
    def on_scroll(self, event):
        ...
        index *= get_voxel_size(self._space_matrix)
        index = index.tolist()
    ...
    def _max_fov(self):
        ...
        mn, mx = compute_fov(self._space_matrix, affines, shapes)
        vx = get_voxel_size(self._space_matrix)
        mn *= vx
        mx *= vx
        return mn, mx
    ...
    def space(self, value):
        ...
        if torch.is_tensor(value):
            ...
            affines = [image.affine for image in self.images]
            default_layout = volume_layout(self.ndim)
            self._space_matrix = affine_reorient(
            ...
            affines = [image.affine for image in self.images]
            voxel_size = get_voxel_size(extra.as_tensor(affines))
